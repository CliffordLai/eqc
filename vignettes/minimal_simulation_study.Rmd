...
As an example, a simulation study is done to compare the performance with regard to the relative error towards the quantile-based classifier (QC) of the following methods.
...
\[
\frac{\text{Err}_c-\text{Err}_\text{quantile}}{(\text{Err}_c+\text{Err}_\text{quantile})/2},
\]
where $\text{Err}_c$ and $\text{Err}_\text{quantile}$ are respectively the test error rate of 
the compared classifier and the quantile-based classifier.
